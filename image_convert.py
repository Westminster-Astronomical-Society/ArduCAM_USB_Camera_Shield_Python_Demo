...
def histeq(image):
    R_, G_, B_ = cv2.split(image)

    R = cv2.equalizeHist(R_)
    G = cv2.equalizeHist(G_)
    B = cv2.equalizeHist(B_)

    return cv2.merge((R, G, B))

def save_image(path, image):
    fn = f"{datetime.now().strftime('%Y-%m-%dT%H%M%S')}.tif"
    Path(path).mkdir(parents=True, exist_ok=True)
    cv2.imwrite(str(Path(path,fn)), image)
